...
from my_specutils.lsf import instrument_lsf_kernel
from my_specutils.spectrum_new import Spectrum
...
class ContinuumModel:
    ...
    def resample_basis_vectors(self, λ_vacuum, v_rel=0.0, Ro=None, Ri=None, telluric=True, **kwargs):
        """
        ...
        λ_vacuum_rest = apply_radial_velocity_shift(λ_vacuum, v_rel)
        
        if Ro is None:
            # Interpolation only.            
                
            basis_vectors = [_interpolate_basis_vector(λ_vacuum_rest, self.basis_vacuum_wavelength, self.stellar_basis_vectors)]
            if telluric:
            ...
            K = instrument_lsf_kernel(self.basis_vacuum_wavelength, λ_vacuum_rest, Ro, Ri)                
    ...
    def get_initial_guess(self, A, flux, ivar, order_masks, n_continuum_params, basis_vectors, p0=None, small=1e-10):
    ...
    def fit(
        ...
        **kwargs
        ...
        # Resample the stellar and telluric basis vectors.
        try:
        ...
        def jacobian(theta):
            return np.hstack([
                ...
                (1 + mBVT @ theta[:n_bases, None]) * A_inv_sigma
            ])

        def f(theta):
            y = (1 - theta[:n_bases] @ basis_vectors) * (A @ theta[n_bases:])
...
if __name__ == "__main__":
    ...
    try:
        ...
        from my_specutils import Spectrum1D
